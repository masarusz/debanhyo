...
### v1.3.4 — 2026-09-08
- The 出番表 title in the header is now a link home, on both pages. On the app
  page it resets the view rather than reloading — a reload would re-fetch the
  whole feed to show a list already in memory — while remaining a real
  `<a href="./">`, so it still works with JS off and supports open-in-new-tab.
